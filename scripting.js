        ...
        <div class="content-container">
            ...
            <div class="button-outercontainer">
                <div class="button-innercontainer">
                    ...
                    <div class="button-div"><a href="main.html" onclick="deletePost('${post.id}')" style="text-decoration: none;">Delete Post</a></div>
...
function findpositioncreate(position) {
    lat = position.coords.latitude;
    lon = position.coords.longitude;
    CreatePost();
}

function findpositionupdate(position) {
    lat = position.coords.latitude
    lon = position.coords.longitude
    editPost()
}

function findlocationupdate() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(findpositionupdate);
    }
    else {
        alert("Location not supported");
    }
...
function findlocationcreate() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(findpositioncreate);
    }
    else {
        alert("Location not supported");
    }
...
function editPost() {
    ...
    fetch(`http://127.0.0.1:8000/api/updatePost/${idput}/`, {
        method: "PUT",
        headers: headers,
        body: JSON.stringify(newPost)
    })
